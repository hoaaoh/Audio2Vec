...
python3 $path/src/audio2vec_eval_all_speaker.py --init_lr=$init_lr --batch_size=$batch_size --seq_len=$seq_len \
  --feat_dim=$feat_dim --p_hidden_dim=$p_dim --s_hidden_dim=$s_dim --stack_num=$stack_num $tf_log_dir $tf_model_dir \
  $feat_dir/all_AE $feat_dir $model_type /nfs/Guanyin/grtzsohalf/yeeee/English
